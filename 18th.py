...
encoder =  Encoder() #torch.load("models/encoder_18") #
decoder =  Decoder() #torch.load("models/decoder_18") #
...
for epoch in range(100000000):
    ...
    npzs = normalize_pattern(zs.detach())
    ndzs = torch.tensor(np.random.normal(0,1,(zs.shape))).float()
    ndzs_p = transfer_pattern(npzs, ndzs)

    gen_imgs2 = decoder(ndzs_p)
    ...
    if epoch % 50 == 0:
        ...
        torch.save(encoder, "models/encoder_18")
        torch.save(decoder, "models/decoder_18")
